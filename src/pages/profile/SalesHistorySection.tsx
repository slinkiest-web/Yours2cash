...
export const SalesHistorySection: React.FC<{ userId: string }> = ({ userId }) => {
  const ordersQuery = useQuery({
    queryKey: ["orders", "seller", userId],
    queryFn: () => fetchSellerOrders(userId),
  })
  const orders = ordersQuery.data?.data ?? []

  return (
    <section className="space-y-4">
      <h2 className="text-xl font-bold text-serif text-text">Sales History</h2>
      {ordersQuery.isLoading ? (
        <div role="status" aria-live="polite" className="space-y-3">
          <span className="sr-only">Loading sales history…</span>
        ...
        <EmptyState
          icon={Wallet}
          title="No sales yet"
          description="Orders buyers place on your listings will show up here."
        />
      ) : (
